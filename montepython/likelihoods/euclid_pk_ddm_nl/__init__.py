...
class euclid_pk_ddm_nl(Likelihood):
    ...
    def loglkl(self, cosmo, data):
        ...
        if self.ddm_non_linear:
            LCDM = Class()
            commonsettings={'omega_b':cosmo.omega_b(), 'omega_cdm':cosmo.omega_cdm()+cosmo.omega_ini_dcdm(),'h':cosmo.h(),'tau_reio':cosmo.tau_reio(),'n_s':cosmo.n_s(),'A_s':cosmo.A_s(),'output':'mPk','non linear':'halofit','z_max_pk': self.zmax,'P_k_max_1/Mpc': 1.5*self.k_cut(self.zmax),'N_ur':2.0328,'N_ncdm':1,'m_ncdm':0.06}
            LCDM.set(commonsettings)
            LCDM.compute()
            pk_nl_lcdm = np.zeros((self.k_size,2*self.nbin+1,self.mu_size),'float64')
            pk_nl_lcdm = LCDM.get_pk_cb(self.k,self.z,self.k_size,2*self.nbin+1,self.mu_size)
            u = cosmo.omega_b()/0.02216
            v = cosmo.h()/0.6776
            w = (cosmo.omega_cdm()+cosmo.omega_b())/0.14116
            ...
            for index_k in xrange(self.k_size):
                for index_z in xrange(self.nbin):
                    for index_mu in xrange(self.mu_size):
                        aa = 0.7208+2.027*(1/cosmo.tau_gyr())+3.031*(1/(1+1.1*self.z_mean[index_z]))-0.18
                        bb = 0.0120+2.786*(1/cosmo.tau_gyr())+0.6699*(1/(1+1.1*self.z_mean[index_z]))-0.09
                        pp = 1.045+1.225*(1/cosmo.tau_gyr())+0.2207*(1/(1+1.1*self.z_mean[index_z]))-0.099
                        qq = 0.9922+1.735*(1/cosmo.tau_gyr())+0.2154*(1/(1+1.1*self.z_mean[index_z]))-0.056
                        epsilon_nl_over_lin = ((1+aa*(self.k[index_k,2*index_z+1,index_mu])**pp)/(1+bb*(self.k[index_k,2*index_z+1,index_mu])**qq))*frac_ddm
                        epsilon_lin = alpha*((1.0/cosmo.tau_gyr())**beta)*(1.0/(0.105*self.z_mean[index_z]+1.0))**gamma
                        pk_nl_th[index_k,2*index_z+1,index_mu] = (1-epsilon_nl_over_lin*epsilon_lin)*pk_nl_lcdm[index_k,2*index_z+1,index_mu]
            LCDM.struct_cleanup()
            LCDM.empty()
     # GFA: Note that pk_nl_th is only evaluated at the z_mean values, not in the edges, even if pk_nl_th has allocated space for mean+edges
        if self.plot_pk_nl_residuals:
            nkbins = range(self.k_size)
#            print("z_mean[0]= %f"%self.z_mean[0])
#            print("mu_fid[0]= %f"%self.mu_fid[8])
#            print("tau_gyr = %f"%cosmo.tau_gyr())
#            print('frac_ddm %f'%(frac_ddm))
            plt.title(r"$z = %.2f, \ \ \ \mu = %.0f $"%(self.z_mean[self.bin_to_plot],self.mu_fid[-1]), fontsize =15)
            plt.text(0.2,0.954,r"$\tau = %.1f \ \mathrm{Gyrs}$"%cosmo.tau_gyr(), fontsize=15)
            plt.text(0.2,0.952,r"$f_{\mathrm{ddm}} = %.1f $"%frac_ddm, fontsize=15)
            plt.plot(self.k[nkbins,2*self.bin_to_plot+1,-1],pk_nl_th[nkbins,2*self.bin_to_plot+1,-1]/pk_nl_lcdm[nkbins,2*self.bin_to_plot+1,-1])
            plt.xlabel(r'$ k \ [h/\mathrm{Mpc}] $', fontsize= 15)
            plt.ylabel(r'$ P_{\mathrm{DDM}}(k)/P_{\Lambda\mathrm{CDM}}(k) $', fontsize=18)
#            plt.xlim([self.k[0,1,8],self.k[-1,1,8]])
            plt.show()
