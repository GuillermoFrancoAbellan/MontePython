...
def chain(cosmo, data, command_line):
    ...
    if (data.get_mcmc_parameters(['varying']) != []):
        ...
        minimum = 0
        if command_line.minimize and not command_line.minimizer =="mcmc":
    ...
    while k <= command_line.N:
        ...
        if command_line.minimize and command_line.minimizer =="mcmc":
          #if k % 50 == 0:
          if acc_loc > 8. or k%400 == 0:
            # check how well we are converging, and optimally decrease lklfactor/jumping factor
            ##print("jump = ",data.jumping_factor)
            rate = acc_loc / (acc_loc + rej_loc)
            acc_loc = 0.0
            rej_loc = 0.0
            ##print("RATE = ",rate)
            if rate>= 0.1:
              command_line.lklfactor*=2
              print("\nMCMC minimizer :: increasing lklfactor to %g\n"%(command_line.lklfactor))
            else:
              data.jumping_factor*=0.7
              print("\nMCMC minimizer :: decreasing jumping factor to %g\n"%(data.jumping_factor))
            # Increase for even more digits of the lkl minimization
            if command_line.lklfactor > 1000:
              ##print("DONE DONE  jump = ",data.jumping_factor)

              # Report the 'current' point to the 'last_accepted'
              sampler.accept_step(data)

              # Write results to results.minimized
              labels = data.get_mcmc_parameters(['varying'])
              for elem in labels:
                data.mcmc_parameters[elem]['current'] = data.mcmc_parameters[elem]['last_accepted']
              data.update_cosmo_arguments()  # this fills in the fixed parameters


              fname = os.path.join(command_line.folder, 'results.minimized')
              with open(fname, 'w') as f:
                  while isinstance(newloglike,(list,np.ndarray,tuple)):
                    newloglike = newloglike[0]
                  f.write('# minimized \chi^2 = {:g} \n'.format(-2.*newloglike))
                  f.write('# %s\n' % ', '.join(['%16s' % label for label in labels]))
                  for elem in labels:
                      bf_value = data.mcmc_parameters[elem]['last_accepted']
                      if bf_value > 0:
                          f.write(' %.6e\t' % bf_value)
                      else:
                          f.write('%.6e\t' % bf_value)
