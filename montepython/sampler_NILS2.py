...
def get_minimum(cosmo, data, command_line, covmat):
    ...
    if command_line.minimizer =="minuit":
      from iminuit import Minuit
      chi2minuit = lambda params: chi2_eff(params, cosmo,data)
      limit_minuit = {'limit_x{}'.format(i):bounds[i] for i in range(len(parameters))}
      minuit_instance = Minuit.from_array_func(chi2minuit, parameters, stepsizes, errordef=1., **limit_minuit)
      minuit_instance.tol = 1
      #minuit_instance.limits = bounds
      result = minuit_instance.migrad()
      resultdict = {'x':[result.params[i].value for i in range(len(result.params))] , 'fun':result.fmin.fval}
    elif command_line.minimizer == "pybobyqa":
      import pybobyqa
      chi2pybobyqa = lambda params: chi2_eff(params, cosmo,data)
      soln = pybobyqa.solve(chi2pybobyqa, parameters, bounds=bounds.T,scaling_within_bounds=True)
      if not (soln.flag == soln.EXIT_SUCCESS):
        raise Exception("Could not minimize using pybobyqa. See solution:"+str(soln))
      resultdict = {'x':soln.x,'fun':soln.f}
    elif command_line.minimizer == "differential":
      result = op.differential_evolution(chi2_eff,
                                         bounds,
                                         args = (cosmo,data))
      resultdict = {'x':result.x,'fun':result.fun}
    elif command_line.minimizer == "shgo":
      result = op.shgo(chi2_eff,
                       bounds,
                       args = (cosmo,data))
      resultdict = {'x':result.x,'fun':result.fun}
    elif command_line.minimizer == "anneal":
      result = op.dual_annealing(chi2_eff,
                                 bounds,
                                 args = (cosmo,data))
      resultdict = {'x':result.x,'fun':result.fun}
    else:
      ...
      try:      
        ...
        raise ValueError("you probably passed an invalid scipy.minimize solver for the minimization, or misspelt one of the other custom solvers")
      resultdict = {'x':result.x,'fun':result.fun}

    print('Final output of minimize')
    for index,elem in enumerate(parameter_names):
        print(elem, 'new:', resultdict['x'][index], ', old:', parameters[index])

    return resultdict['x'], resultdict['fun']
